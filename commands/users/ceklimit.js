...
m.reply(`Limit left: ${user.checkPremiumUser(m.mentions[0], _user) ? "Unlimited" : `${user.getLimit(m.mentions[0], _user)} / ${config. options.limitCount} Max\nLimit is reset every 06:00 WIB\n`
            }\nBalance : $${user.getBalance(m.mentions[0],_user)}`)
} else {
m.reply(`Limit left: ${user.checkPremiumUser(m.sender, _user) ? "Unlimited" : `${user.getLimit(m.sender, _user)} / ${config.options.limitCount} Max\ nLimit is reset every 06:00 WIB\n`
